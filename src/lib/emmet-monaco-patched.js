...
function isOperator$1(token, type) {
  return Boolean(
    token && token.type === "Operator" && (!type || token.operator === type),
  );
}
    ...
    children: convertGroup(abbr, {
      ...
      getText(pos) {
        ...
        if (Array.isArray(options.text)) {
          ...
          value =
            pos !== undefined ? options.text[pos] : options.text.join("\n");
        } else {
...
function convertStatement(node, state) {
  ...
  if (node.repeat) {
    ...
    result = result.concat(
      isGroup(node) ? convertGroup(node, state) : convertElement(node, state),
    );
  }
  return result;
}
...
function isField$1(token) {
  return (
    typeof token === "object" && token.type === "Field" && token.index != null
  );
}
function last$1(arr) {
...
function shouldConsumeDashAfter(token) {
  return (
    token.type === "ColorValue" || (token.type === "NumberValue" && !token.unit)
  );
}
/**
...
function isBracket(token, open) {
  return (
    token && token.type === "Bracket" && (open == null || token.open === open)
  );
}
...
function resolveImplicitTag(node, ancestors, config) {
  ...
  node.name =
    elementMap[parentName] || (isInline(parentName, config) ? "span" : "div");
}
...
function getIndent(state) {
  ...
  if (
    !parent ||
    isSnippet(parent) ||
    (parent.name && config.options["output.formatSkip"].includes(parent.name))
  ) {
    return 0;
  }
  return 1;
...
function asHex(token, short) {
  const fn =
    short && isShortHex(token.r) && isShortHex(token.g) && isShortHex(token.b)
      ? toShortHex
      : toHex;
  ...
  if (!resolveGradient(node, config)) {
    ...
    if (isValueScope(config)) {
      ...
      const snippet = snippets.find(
        (s) => s.type === CSSSnippetType.Property && s.property === propName,
      );
  ...
  for (const cssVal of node.value) {
    ...
    for (const token of cssVal.value) {
      if (token.type === "Literal") {
        value.push(
          resolveKeyword(token.value, config, snippet, minScore) || token,
        );
      } else if (token.type === "FunctionCall") {
...
function resolveKeyword(kw, config, snippet, minScore) {
  ...
  if (
    (ref = findBestMatch(kw, config.options["stylesheet.keywords"], minScore))
  ) {
    return literal(ref);
  }
  return null;
...
function wrapWithField(node, config, state = { index: 1 }) {
  ...
  for (const v of node.value) {
    switch (v.type) {
      case "ColorValue":
        value.push(
          field(state.index++, color(v, config.options["stylesheet.shortHex"])),
        );
        break;
      case "Literal":
...
function isOpenBracket(ch) {
  return (
    ch === Brackets.CurlyL || ch === Brackets.RoundL || ch === Brackets.SquareL
  );
}
function isCloseBracket(ch) {
  return (
    ch === Brackets.CurlyR || ch === Brackets.RoundR || ch === Brackets.SquareR
  );
}
...
function isOpenBrace(ch, syntax) {
  return (
    ch === Brackets.RoundL ||
    (syntax === "markup" && (ch === Brackets.SquareL || ch === Brackets.CurlyL))
  );
}
function isCloseBrace(ch, syntax) {
  return (
    ch === Brackets.RoundR ||
    (syntax === "markup" && (ch === Brackets.SquareR || ch === Brackets.CurlyR))
  );
}
...
function doComplete(monaco, model, position, syntax, emmetConfig) {
  ...
  if (!isStyleSheetRes) {
    if (!snippetKeyCache.has(syntax)) {
      const registry = Object.assign(
        Object.assign({}, getDefaultSnippets(syntax)),
        customSnippetsRegistry[syntax],
      );
      snippetKeyCache.set(syntax, Object.keys(registry));
    }
    markupSnippetKeys =
      (_a = snippetKeyCache.get(syntax)) !== null && _a !== void 0 ? _a : [];
  }
  ...
  const { abbreviationRange, abbreviation, currentLineTillPosition, filter } =
    extractedValue;
  ...
  const createExpandedAbbr = (syntax, abbr) => {
    ...
    if (
      !expandedText ||
      isExpandedTextNoise(syntax, abbr, expandedText, expandOptions.options)
    ) {
      return;
    }
    expandedAbbr = {
  ...
  if (isStyleSheetRes) {
    ...
    if (
      ...
      cssData.properties.some((x) => x.startsWith(abbreviation))
    ) {
      return { suggestions: [], incomplete: true };
    }
    if (expandedAbbr && expandedText.length) {
      expandedAbbr.range = abbreviationRange;
      expandedAbbr.insertText = escapeNonTabStopDollar(
        addFinalTabStop(expandedText),
      );
      ...
      completionItems = makeSnippetSuggestion(
        monaco,
        stylesheetCustomSnippetsKeys !== null &&
          stylesheetCustomSnippetsKeys !== void 0
      ...
      if (
        !completionItems.find(
          (x) =>
            x.insertText ===
            (expandedAbbr === null || expandedAbbr === void 0
              ? void 0
              : expandedAbbr.insertText),
        )
      ) {
  ...
  return completionItems.length
    ? { suggestions: completionItems, incomplete: true }
    : undefined;
}
/**
...
function isExpandedTextNoise(syntax, abbreviation, expandedText, options) {
  ...
  if (isStyleSheet(syntax) && options) {
    const between =
      (_a = options["stylesheet.between"]) !== null && _a !== void 0
        ? _a
        : ": ";
    const after =
      (_b = options["stylesheet.after"]) !== null && _b !== void 0 ? _b : ";";
  ...
  if (syntax === "jsx") {
    ...
    return (
      ...
      [
        ...
        "type.identifier.javascriptreact",
        "identifier.typescriptreact",
        "type.identifier.typescriptreact",
...
function getTokenizationEnv(model) {
  ...
  let _tokenizationStateStore =
    _tokenization === null || _tokenization === void 0
      ? void 0
...
function isValidLocationForEmmetAbbreviation(
  model,
  position,
  syntax,
  language,
